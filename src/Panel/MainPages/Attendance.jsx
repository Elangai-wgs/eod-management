...
//             ...new Set(res.data.data.map((item) => item.auth?.role)),
//           ];
...
const Attendance = () => {
    ...
    const fetchAttendanceData = async () => {
      try {
        ...
        if (res?.data?.data) {
          setAttendanceData(res.data.data);
          console.log(res.data.data);
          const extractedRoles = res.data.data
            .map((item) => item.auth)
            .filter((auth) => auth && auth.role)
            .map((auth) => ({
              _id: auth.role._id,
              roleName: auth.role.roleName,
            }));
  ...
  // Filter data based on search query, date, and selected role
  const filteredData = (attendanceData || []).filter((item) => {
    // Check if the user name matches the search query
    const matchesSearch = item.auth?.fullName
      ?.toLowerCase()
      .includes(searchQuery.toLowerCase());
  ...
  ? item.dateString &&
    (() => {
      const [day, month, year] = item.dateString.split("/"); // Split the date string into parts
      const formattedDate = `${year}-${month}-${day}`; // Reformat to YYYY-MM-DD
      return formattedDate === selectedDate;
    })()
  : true;

console.log("Item Date String:", item.dateString);
...
console.log("Matches Date:", matchesDate);
    ...
    const matchesRole = selectedRole
      ? item.auth?.role?.roleName?.trim().toLowerCase() === selectedRole.trim().toLowerCase()
      : true;
...
console.log("");
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row) => (
        <img
          src={row.auth?.profilePic || "/placeholder.jpg"}
          alt="Profile Pic"
          className="w-10 h-10 rounded-full"
        />
      ),
    ...
    {
      name: "Name",
      selector: (row) => <p className="text-sm">{row.auth?.fullName || "N/A"}</p>,
      sortable: true,
      center: true,
    },
    {
      name: "Role",
      selector: (row) => (
        <p className="text-sm">{row.auth?.role?.roleName || "N/A"}</p>
      ),
      sortable: true,
      center: true,
    },
  ...
  const exportToExcel = (data, fileName = "attendance.xlsx") => {
    const processedData = data.map((item) => ({
      fullName: item.user?.fullName || "N/A",
      role: item.auth?.role?.roleName || "N/A",
      status: item.status || "N/A",
      checkIn: new Date(item.checkIn).toLocaleTimeString() || "N/A",
      checkOut: new Date(item.checkOut).toLocaleTimeString() || "N/A",
    }));
      ...
      {isAllowedTo(permission, ["view", "viewOwn"]) ? (
        <div className="container mx-auto p-4">
          ...
          {selectedRole && attendanceData.length > 0 && (
            <DataTable
              columns={columns}
              data={filteredData} // Ensure this is recalculated on change
              pagination
              customStyles={customStyles}
            />
          )}
          ...
          >
            <div className="flex flex-col gap-4">
              <button
                className="bg-orange-500 text-white p-2 px-4 rounded-lg"
                onClick={() => handleExport("today")}
              >
                Export Today
              </button>
